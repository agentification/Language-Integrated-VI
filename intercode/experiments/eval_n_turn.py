...
parser.add_argument('--template', choices=['v1', 'v2', 'game', 'game_sql'],
...
class ExperimentWrapper():
    def __init__(self, args):
        ...
        log_file_name = f"{self.env.name}_multiturn_{args.model}_{args.max_turns}_turns_{args.data_path[-6]}.json"
        ...
        if args.policy == 'chat':
            ...
            self.policy = CompletionGPTPolicy(language=args.env, setting=SETTING_MAP[args.env], template=args.template,
                                              dialogue_limit=args.dialogue_limit, model=args.model)
        else:
